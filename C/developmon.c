...
void printImage(int imageNum){
    FILE *heal,*atk,*atked,*potion1,*potion2,*potion3,*potion4,*potion5,*potion6,*potion7,*run,*event,*root,*levelup,*die,*victory,*init;
    char line[100];
    if(imageNum==1){
        heal = fopen("밥","r");
...
void bossBattle(){
    ...
    while(bossDevmon.hp>0){
        ...
        //체력 회복 추가!
        if(bossDevmon.hp<=500&&bossDevmon.hp>0){
            bossHeal=rand()%100;
            if(bossHeal<30){
    ...
    while (enemyDevmon.hp>0)
    {
        ...
        if(myDevMon1.hp>0){
            ...
            if(choiceAction1==1){
                ...
                system("clear");
                printf("앗! mp가 부족하다! 적 %s는 이상함을 눈치챘다!",enemyDevmon.name);
                sleep(1);
            }
            else if(choiceAction1==3){
                useItem(2);
